    <div class="operate-btn-group">
        <el-button @click="add" v-if="permissions['${module}:${targetTableName}:add']" type="primary" icon="el-icon-plus" size="small">添加</el-button>
<#if table.validStatusColumn??>
        <el-button @click="enableSelected" v-if="permissions['${module}:${targetTableName}:enable'] && multipleSelection.length > 0" type="success" icon="el-icon-check" size="small">启用</el-button>
        <el-button @click="disableSelected" v-if="permissions['${module}:${targetTableName}:disable'] && multipleSelection.length > 0" type="warning" icon="el-icon-close" size="small">禁用</el-button>
</#if>
<#if table.hasUniPk>
        <el-button @click="delSelected" v-if="permissions['${module}:${targetTableName}:del'] && multipleSelection.length > 0" type="danger" icon="el-icon-delete" size="small">删除</el-button>
</#if>
    </div>
...
<#list table.columns as column>
        ...
        <#else>
            ...
            <el-table-column fixed="right" label="操作" width="<#if table.validStatusColumn??>200<#else>170</#if>">
                <template slot-scope="scope">
                    <el-tooltip effect="dark" :open-delay="500" content="编辑" placement="top" v-if="permissions['${module}:${targetTableName}:update']">
                        <el-button @click="edit(scope.row, false)" type="primary" icon="el-icon-edit" size="mini" circle></el-button>
                    </el-tooltip>
                    <el-tooltip effect="dark" :open-delay="500" content="复制" placement="top" v-if="permissions['${module}:${targetTableName}:add']">
...
<#if table.validStatusColumn??>
                    <el-tooltip effect="dark" :open-delay="500" content="禁用" placement="top" v-if="permissions['${module}:${targetTableName}:disable'] && scope.row.${table.validStatusColumn.targetName?uncap_first}==${table.validStatusColumn.validStatusOption.valid}">
                        <el-button @click="disable(scope.row)" type="warning" icon="el-icon-close" size="mini" circle></el-button>
                    </el-tooltip>
                    <el-tooltip effect="dark" :open-delay="500" content="启用" placement="top" v-else-if="permissions['${module}:${targetTableName}:enable'] && scope.row.${table.validStatusColumn.targetName?uncap_first}==${table.validStatusColumn.validStatusOption.invalid}">
                        <el-button @click="enable(scope.row)" type="success" icon="el-icon-check" size="mini" circle></el-button>
                    </el-tooltip>
</#if>
                    <el-tooltip effect="dark" :open-delay="500" content="删除" placement="top" v-if="permissions['${module}:${targetTableName}:del']">
